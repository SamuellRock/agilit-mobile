...
export default function CredorHome() {
  ...
  const totalActive = credorActiveLoans.reduce(
    (sum, loan) => sum + loan.amount,
    0
  );
  const totalReceived = credorHistoricLoans.reduce(
    (sum, loan) => sum + loan.amount,
    0
  );
      ...
      >
        <View className="rounded-3xl bg-dark-300 p-5 shadow-card">
          ...
          <Text className="mt-2 text-sm text-gray-300">
            Última atualização há 3 minutos. Ricardo já respondeu 2 contatos
            hoje.
          </Text>
          ...
          <View className="mt-4 flex-row gap-3">
            <UiButton
              className="flex-1"
              onPress={() => router.push("/credor/offer")}
            >
              Criar oferta
            </UiButton>
            <UiButton
              variant="secondary"
              className="flex-1"
              onPress={() => router.push("/credor/history")}
            >
              Ver histórico
            </UiButton>
          </View>
        ...
        <View className="mt-8">
          <View className="mb-3 flex-row items-center justify-between">
            ...
            <Text className="text-xs uppercase text-gray-400">
              {credorActiveLoans.length} contratos
            </Text>
          </View>
        ...
        <View className="mt-6 rounded-3xl bg-white/5 p-5">
          ...
          <Text className="text-sm text-gray-300">
            Histórico favorece juros médios de 4,5% a.m.
          </Text>
        ...
        <View className="mt-8 mb-8 rounded-3xl bg-dark-300 p-5">
          ...
          {activityTimeline.map((item) => (
            <View
              key={item.id}
              className="mt-4 border-l-2 border-primary/40 pl-4"
            >
